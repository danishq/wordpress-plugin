...
- Authenticates via **`POST …/api/v1/wp/users/login`** (`ADFLIPR_API_WP_USERS_LOGIN_URL`) with email, password, **`context`: `WORDPRESS`**, and WooCommerce API key material in **`queryParams`**. This matches the backend **`WordPressController`** (not **`POST /api/v1/users/login`**, which is the web-app JWT flow). The API returns **`type`: `wp_token`** and a **`token`** field; the plugin stores those like the legacy **`loginToken`** response when needed.
...
- Registers custom REST routes under **`adflipr/v1`**:
  - **`GET /login-token`** — returns token if valid; **`permission_callback`**: **`manage_options`**, valid **`X-WP-Nonce`** (`wp_rest`), **`api_permissions_check`** (referer must start with `site_url()`), and an existing integration token.
  ...
  - **`/proxy/(?P<path>.*)`** — forwards-method requests to **`https://adflipr.com/api/v1/{path}`** with stored token; **`manage_options`** + REST nonce + referer + valid integration token. Responses are normalized to **`WP_REST_Response`**: decoded JSON body (or a small wrapper if upstream is not JSON) and the upstream HTTP status code — not the raw **`wp_remote_*`** array.
